...
export const listarTutores = async (req: Request, res: Response): Promise<void> => {
    try {
...
export const crearPerfilTutor = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const userId = req.usuario!.id;

        const existente = await prisma.tutor.findUnique({ where: { id: userId } });
        if (existente) {
...
export const obtenerTutor = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
